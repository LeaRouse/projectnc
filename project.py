...
# --- Capturar mensajes de botones flotantes ---
components.html("""
<script>
const leftButtons = {
    "btn-left1": "Home",
...
for (const id in leftButtons){
    const el = document.getElementById(id);
    if(el){
        el.onclick = () => {
            window.parent.postMessage({type:leftButtons[id]}, "*");
        }
    }
}
...
# Detectar cambios usando Streamlit buttons normales
botones = {
    "Home": "🏠 Home",
    "Craft": "🛠️ Craft",
...
for key in botones:
    if st.button(botones[key], key=key):
        cambiar_pagina(key)
...
if pagina == "Home":
    ...
    <div style="
        ...
        <h1 style="
            ...
            margin-bottom:0px;">AstroCycle</h1>
    """, unsafe_allow_html=True)
    if IMG_FILE.exists():
